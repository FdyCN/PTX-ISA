...
原文
PTX 程序可在采用 16 位或 32 位数据通路（data path）的 GPU 上执行。当在 32 位数据通路上执行时，PTX 中的 16 位寄存器会映射到 32 位物理寄存器，且 16 位运算会被提升为 32 位运算。这可能导致同一代码在 16 位机器与 32 位机器上运行时出现计算差异 —— 因为被提升后的运算中，寄存器的高半字（high-order half-word）可能存在 16 位物理寄存器中没有的位。这些额外的精度位可能会在应用层显现，例如通过右移指令（right-shift instruction）就能观察到。
在 PTX 语言层面，一种解决方案是为 16 位代码定义与 16 位数据通路执行逻辑一致的语义。但这种方式会给 “在 32 位数据通路上执行的 16 位代码” 带来性能损耗：因为转换后的代码需要添加大量额外的掩码指令（masking instructions），以屏蔽 32 位寄存器高半字中多余的精度位。
PTX 并未选择给 “在 32 位 GPU 上运行的 16 位代码” 引入性能损耗，而是将 16 位指令的语义设计为 “与机器相关”（machine-specific）。编译器或开发者可选择通过在程序的适当位置添加显式的 16 位值转换操作，来强制实现可移植、与机器无关的 16 位语义，从而保证代码的可移植性。然而，对于许多性能敏感型应用而言，这种做法并不理想；并且对多数应用来说，接受执行结果的差异，比限制性能更为可取。
## 9.7.  Instructions
